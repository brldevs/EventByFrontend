...
function EventTicketForm({
  ...
  const onSubmit = async (d) => {
    console.log("inputFields[0].currency");
    console.log(inputFields[0].currency);
  ...
  const handleAddFields = () => {
    ...
    console.log("My custom value : " + inputFields[0].currency);
    if (!inputFields[0].name || !inputFields[0].price) {
      setIsCustomTicketError(true);
    } else {
      if (isTicketNameSame()) {
        setCustomTicketErrorMsg("Ticket name can not be same");
