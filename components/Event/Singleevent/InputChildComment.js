import { eventPostChildComment } from "../../../services/service";
import React, { useEffect, useState, useCallback } from "react";
import { useForm } from "react-hook-form";
function InputChildComment({
  token,
  eventId,
  setIsLoading,
  parentId,
  hideIsReplyInputShow,
  index,
}) {
  ...
  const onSubmit = async () => {
    if (commentDetails) {
      ...
      const res = await eventPostChildComment(data, token, mediaFiles);
      if (res.status === 200) {
        setIsDisableSubmitCommentBtn(false);
      }
      setIsLoading(true);
      hideIsReplyInputShow(index);
    }
  };
      ...
      <div className="ps-3 ps-md-5 ms-3 mt-4 mt-md-5 mb-5">
        ...
        <div className="comment-attachment">
          <div className="d-flex flex-wrap mt-2">
            ...
            <div>
              <label htmlFor={`childCommentPicsId-${index}`}>
                <img
